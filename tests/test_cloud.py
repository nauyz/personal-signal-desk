    ...
    def test_export_has_all_views_and_never_accesses_network(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(server, 'DB_PATH', root / 'test.db'), \
                ...
                server.AIHotClient._state('producthunt:categories', None, 'error', 'HTTP 403: access restricted')
                cloud_build.export()
                ...
                self.assertEqual(data['launches:productivity']['sourceStatus'], 'access-restricted')
